...
@section('page_title',(!is_null($research) && strlen($research->title) > 1 ? $research->title : __('general.topAnalizes')))
@section('seo_title', (!is_null($research) && strlen($research->seo_title) > 1 ? $research->seo_title : __('general.topAnalizes')))
@section('meta_keywords',(!is_null($research) && strlen($research->meta_keywords) > 1 ? $research->meta_keywords :__('general.topAnalizes')))
@section('meta_description', (!is_null($research) && strlen($research->meta_description) > 1 ? $research->meta_description : __('general.topAnalizes')))
...
@section('content')
    <div class="container-hc">
        <div class="bread">
            @include('partials.breadcrumbs',['title'=>$page->title,'titleLink'=>route('pages.get',$page->slug),'subtitle'=> !is_null($research)? $research->title : __('general.topAnalizes')])
        </div>
        <div class="title-main">
            <img src="/img/icons/syringe-links-icon.svg" alt="syringe">
            <h3>{{$page->title.' - '.(!is_null($research)? $research->title : __('general.topAnalizes'))}}</h3>
        </div>
        <section class="analyses-all">
            <form action="{{route('analyzes.research','top')}}" method="GET" class="services-inner__search page__search">
            ...
            <div class="analyses-all__content">
                <ul class="nav nav-tabs analyses-all__left" data-mcs-theme="dark" id="myTab" role="tablist">
                    <li class="nav-item">
                        <a class="nav-link @if(is_null($research)) active @endif"
                           href="{{route('analyzes.research','top')}}">@lang('general.topAnalizes')
                        </a>
                    </li>
                    @foreach($researches as $rsrch)
                        <li class="nav-item">
                            <a class="nav-link @if(!is_null($research) && $research->id === $rsrch->id) active @endif"
                               href="{{route('analyzes.research',$rsrch->slug)}}">{{$rsrch->title}}</a>
                        </li>
                    @endforeach
                </ul>
                <div class="tab-content analyses-all__right" id="myTabContent">
                    <div id="acc">
                        @foreach($analyzes as $analis)
                            <div class="card">
                                <div class="card-header" id="h{{$analis->id}}">
                                    <h5 class="mb-0">
                                        <button class="btn btn-link collapsed" data-toggle="collapse" data-target="#tab{{$analis->id}}"
                                                aria-expanded="false" aria-controls="tab{{$analis->id}}">
                                            <div class="acc-arr">
                                            ...
                                            <span>{{$analis->title}}</span>
                                        </button>
                                    </h5>
                                </div>

                                <div id="tab{{$analis->id}}" class="collapse" aria-labelledby="h{{$analis->id}}" data-parent="#acc">
                                    <div class="card-body">
                                        <div class="services-inner__info">
                                            ...
                                            <div class="info__price">
                                                ...
                                                <span class="info__qty">{{number_format($analis->price,0,'',' ')}} @lang('general.unit')</span>
                                            </div>
                                            <a href="#" class="btn-request c-modal" data-page="{{$analis->title}}">@lang('general.signup')</a>
                                            <add-to-cart product-id="{{$analis->id}}"
                                                         btn-class="btn-request add-to-cart-btn"
                                                         btn-add="@lang('general.addToCart')" btn-added="@lang('general.addedToCart')" is-added="{{\Cart::has($analis->id)}}"></add-to-cart>
                                        </div>
                                        <div class="analyses-all__text">{!! $analis->content !!}</div>
                                    </div>
                                </div>
                            </div>
                        @endforeach
