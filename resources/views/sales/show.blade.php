...
@section('content')
    <div class="container-hc">
        <div class="bread">
            @include('partials.breadcrumbs',['title'=>$page->title,'titleLink'=> route('sales.month'), 'subtitle'=>$sale->title])
        </div>
        ...
        <section class="stock__menu">
            @if($daySaleExistance)
                <a href="{{route('sales.index')}}" class="stock__menu-btn @if($sale->type === 'day') stock__menu-btn_active @endif">@lang('general.salesOfDay')</a>
            @endif
            <a href="{{route('sales.month')}}" class="stock__menu-btn @if($sale->type === 'month') stock__menu-btn_active @endif">@lang('general.salesOfMonth')</a>
        </section>
    </div>
    <section class="stock__info-item">
        <div class="stock__info-head" @if(strlen($sale->image) > 0) style="background-image: url({{\Voyager::image($sale->image)}});" @endif>
